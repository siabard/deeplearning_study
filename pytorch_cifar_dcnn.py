...
train_dataset = datasets.CIFAR10('./CIFAR10_data', train=True, download=True, transform=transforms.ToTensor())
test_dataset = datasets.CIFAR10('./CIFAR10_data', train=False, download=True, transform=transforms.ToTensor())
...
class DeepLearningModel(nn.Module):
    ...
    def forward(self, data):
        data = self.conv1(data)   # (32, 32, 3)
        data = torch.relu(data)   # (32, 32, 32)
        data = self.conv2(data)   # (32, 32, 32)
        data = torch.relu(data)   # (32, 32, 32)
        data = self.pooling(data) # (16, 16, 32)
        data = self.dropout25(data) # (16, 16, 32)

        data = self.conv3(data)     # (16, 16, 32)
        data = torch.relu(data)     # (16. 16. 64)
        data = self.conv4(data)     # (16, 16, 64)
        data = torch.relu(data)     # (16, 16, 64)
        data = self.pooling(data)   # (8, 8, 64)
        data = self.dropout25(data) # (8, 8, 64)
        
        data = self.conv5(data)     # (8, 8, 128)
        data = torch.relu(data)     # (8. 8. 128)
        data = self.pooling(data)   # (4, 4, 128)
        data = self.dropout25(data) # (4, 4, 128)

        data = self.conv6(data)     # (4, 4, 128)
        data = torch.relu(data)     # (4. 4. 128)
        data = self.pooling(data)   # (2, 2, 128)
        data = self.dropout25(data) # (2, 2, 128)

        data = self.conv7(data)     # (2, 2, 256)
        data = torch.relu(data)     # (2. 2. 256)
        data = self.pooling(data)   # (1, 1, 256)
        data = self.dropout25(data) # (1, 1, 256)

        data = data.view(-1, 1 * 1 * 256) # Flatten the tensor into a vector
        data = self.fc1(data)
